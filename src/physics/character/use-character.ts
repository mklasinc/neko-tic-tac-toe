...
import { RapierRigidBody, useAfterPhysicsStep, useBeforePhysicsStep, useRapier, vec3 } from '@react-three/rapier'
    ...
    if (body) {
      try {
        ...
        if (ray) {
          ...
          const intersection = rapier.world.raw().castRay(ray, 0.01, true)
          ...
          console.log('grounded', characterState.grounded)
        }

        if (characterState.grounded && keysdown.current[' '] && !characterState.jumping) {
